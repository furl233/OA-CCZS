            ...
            <v-row dense>
                <v-col cols="12" md="12"> 
                    <v-card dense color="white" min-height="700">
                        <v-card-title class="headline indigo white--text">材料合同查找
                            <v-spacer></v-spacer>
                            ...
                            <template v-slot:top>
                                <v-toolbar flat>
                                    ...
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">新增材料合同</v-btn>
                                    </template>
                                    <v-card>
                                        ...
                                        <v-card-text>
                                          <div class="ma-3"> 
                                            ...
                                            <v-row dense>
                                              <v-col cols="12" md="12" v-if="editedIndex <= -1">
                                                  <v-autocomplete dense :items="suppliers" return-object item-text="supplier_name" item-value="supplier_name" label="材料供应商" hint="材料供应商" prepend-icon="mdi-account" v-model="select.suppliers"
                                                    solo :search-input.sync="search_temp.suppliers"></v-autocomplete>
                                              </v-col>
                                              <v-col cols="12" md="12" v-if="editedIndex > -1">
                                                  <v-text-field  dense label="材料供应商" disabled outlined prepend-icon="mdi-account" v-model="editedItem.supplier_name"></v-text-field>
                                              </v-col>
                                              <v-col cols="12" md="8">
                                                   <v-text-field dense label="材料金额" type="number" hint="材料金额" outlined prepend-icon="mdi-bank" v-model="editedItem.total_amount"></v-text-field>
                                              </v-col>
                                              <v-col cols="12" md="4">
...
export default {
    ...
    data(){
        return{
            ...
            profiles: [],
            editedIndex: -1,
            editedItem: {purchase_purpose:"材料"},
            defaultItem: {purchase_purpose:"材料"},
            ...
            headers: [{text: '工程名称',align: 'start',sortable: false,value: 'contract_name',},
                    ...
                    { text: '材料合同总额(含税)', value: 'total_amount'},
    ...
    methods:{
        ...
        async initialize () {
            await this.$http.get(`http://localhost:3000/purchase?per_page=1000&page=1&keyword=&typekeyword=材料`)
        ...
        getsuppliers(val){
            let seqNumber = ++this.axioscount;
            this.$http.get(`http://localhost:3000/supplies?per_page=10&page=1&keyword=${val}&typekeyword=材料`)
                .then(res=> {
                    if (seqNumber === this.axioscount) {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? '新增采购合同' : '修改采购合同'
      },
    },
    created(){
      this.initialize()
